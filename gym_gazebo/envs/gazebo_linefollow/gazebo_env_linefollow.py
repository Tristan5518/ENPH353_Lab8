...
class Gazebo_Linefollow_Env(gazebo_env.GazeboEnv):
    ...
    # Get image dimensions
        ...
        # Select the row at 90% of the image height
        target_row = int(0.9 * height)
        ...
        #Make Binary to help  make simpler
        res, binIm = cv2.threshold(grayIm, 150, 255, cv2.THRESH_BINARY_INV)


         # Show both raw and processed images
        cv2.imshow("Robot View", cv_image)
        cv2.imshow("Filtered Image", binIm)
        cv2.waitKey(1)  # Ensure images refresh


        #PID loop to find the direction to go
        start = 25
        end = 25
        init = False
        for i, frame in enumerate(binIm[int(target_row),:]):
            if frame != 0:
                if not init:
                    start = i
                    init = True
        ...
        target = int(end - (end - start)/2)

        if(init == True):
            self.timeout = 0
            targIdx = math.floor(float(float(target)/width) * 9)
            state[targIdx] = 1
            return state, done
        else:
